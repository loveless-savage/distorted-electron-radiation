...
if show3dFarField
    ...
    Nt_tot=zeros(sph_res+1,sph_res+1);
    Np_tot=Nt_tot;
    ...
    for k=1:sph_res+1 % theta (latitude)
        ...
        for j=1:sph_res+1 % phi (longitude)
            ...
            ft=trapz(nu,At_bp)*9.00e-4;
            fp=trapz(nu,Ap_bp)*9.00e-4;
            Nt_tot(k,j)=ft/(1.55*(fmax+fmin)/2);
            Np_tot(k,j)=fp/(1.55*(fmax+fmin)/2);
        end
    end
    figure;
    ...
    surf(X,Y,Z,Np_tot,'Edgecolor', 'none')
    xlabel('x - axis')
    ylabel('y - axis')
    zlabel('z - axis')
    shading interp
    ...
    figure;
    colormap([0 0 0;0 0 .25;0 0 .5;0 0 .75;0 0 1;0 .25 .75;0 .5 .5;0 .75 .25;0 1 0;.25 .75 0;.5 .5 0;.75 .25 .33;1 0 0;1 .25 .25;1 .5 .5;1 .75 .75;1 1 1]);
    surf(X,Y,Z,Nt_tot,'Edgecolor', 'none')
    xlabel('x - axis')
    ylabel('y - axis')
    zlabel('z - axis')
    shading interp
